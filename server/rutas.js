...
Router.post('/events/new', function(req, res) {
 ...
 Router.post('/events/update', function(req, res) {
   //Si no hay usuario logueado, no permite actualizar una fecha
   if (!req.session.email) res.send(false)
   else{
     Events.update(
                    {_id: req.body._id},
  ...
  Router.post('/events/delete/:id', function(req, res) {
    //Si no hay usuario logueado, no permite eliminar una evento
    if (!req.session.email) res.send(false)
    else{
      let eid = req.params.id
      Events.remove({_id: eid}, function(error) {
          if(error) {
              res.status(500)
              res.json(error)
          }
...
Router.get('/events/all', function(req, res) {
  //Si no hay usuario logueado, no permite ver eventos
  if (!req.session.email) res.send("logout")
  else{
    Events.find({user_email: req.session.email}).lean().exec(function(err, eventos) {
